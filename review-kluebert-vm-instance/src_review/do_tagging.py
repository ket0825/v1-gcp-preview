...
from modeling.trainer import tag_valid_fn, valid_eval_fn, tag_fn, preprocess_fn, preprocess_fn_deploy ### 함수import 수정
...
def tag_valid(config):
    ...
    enc_sentiment, enc_aspect, enc_aspect2 = meta_data["enc_sentiment"], meta_data["enc_aspect"], meta_data["enc_aspect2"] ### enc_aspect2 추가함
    enc_sentiment_score, enc_aspect_score = meta_data["enc_sentiment_score"], meta_data["enc_aspect_score"] ### score encoder 추가
    ...
    num_sentiment_score = len(list(meta_data["enc_sentiment_score"].classes_)) ### score_num 추가
    num_aspect_score = len(list(meta_data["enc_aspect_score"].classes_)) ### score_num 추가
    ...
    model = ABSAModel(num_sentiment=num_sentiment, num_aspect=num_aspect, num_aspect2=num_aspect2,
    ...
    # 태깅 수행
    tag_valid_fn(config, tokenizer, model, enc_sentiment, enc_aspect, enc_aspect2, enc_sentiment_score, enc_aspect_score,
            device, log)
...
def tag(config):
    ...
    # eval_data_loader = set_loader(config=config, fp=tagging_fp, batch_size=config.eval_batch_size, meta_data=meta_data)

    tokenizer = transformers.BertTokenizer.from_pretrained(config.init_model_path, do_lower_case=False)
    # 태깅 수행
    tag_fn(config, tokenizer, model, enc_sentiment, enc_aspect, enc_aspect2, enc_sentiment_score, enc_aspect_score,
            device, log)
    ### eval_fn 형식에 맞게 score 추가
...
def preprocess(config):
    ...
    start_time_preprocessing = datetime.datetime.now()
    preprocess_fn(config)
    end_time_preprocessing = datetime.datetime.now()

    print(f"전처리 시간: {end_time_preprocessing - start_time_preprocessing}")
    
def preprocess_deploy(
        packet: Dict[str, Any]
    ) -> None: 
    """
    Get packet and Post request to ther server
    
    {
    "type": "R0",
    "category": "chicken",
    "prid": "P0PF",
    "match_nv_mid": "47926075623",
    "reviews": [
        {
            "id": "str",
            "content": "str",
            "aidaModifyTime": "strftime",
            "mallId": "str",
            "mallSeq": "str",
            "matchNvMid": "str",
            "nvMid": "str",
    ...
    Args:
        packet (Dict[str, Any]): get raw packet

    Returns:
        packet Dict[str, Any]: return preprocessed review packet
    """

    preprocessed_review_list = preprocess_fn_deploy(packet['reviews'])
    packet['reviews'] = preprocessed_review_list
    ...
...
if __name__ == "__main__":
    ...
    parser.add_argument("--eval_batch_size", type=int, default=2, help="한 batch에 속할 테스트 데이터 샘플의 size")
    ...
    # ===================================================
    parser.add_argument("--preprocessing_fp", type=str, help="전처리할 데이터들이 포함된 디렉토리 경로 or 전처리할 데이터 파일 경로 설정", default="./resources_review/data_json/")    
    parser.add_argument("--tagging_fp", type=str, help="태깅할 데이터들이 포함된 디렉토리 경로 or 태깅할 데이터 파일 경로 설정", default="./resources_review/preprocessed_results_json/")
    ### path 수정
    parser.add_argument("--valid_fp", type=str, help="valid 데이터 포함된 디렉토리 경로 or valid 데이터 파일 경로 설정", default="./resources_review/data_valid/")
    parser.add_argument("--base_path", type=str, help="평가를 수행할 Model과 Encoder가 저장된 경로", default="./ckpt_review/model/")
    ### 경로 수정
    ...
    # json 파일들에 대해서 모델이 예측한 결과를 json으로 저장
    tag(config)
