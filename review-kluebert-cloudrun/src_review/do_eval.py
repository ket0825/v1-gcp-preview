...
def eval(config):
    ...
    enc_sentiment, enc_aspect = meta_data["enc_sentiment"], meta_data["enc_aspect2"]
    ...
    model = ABSAModel(num_sentiment=num_sentiment, num_aspect=num_aspect, num_aspect2=num_aspect2, config=config,
    ...
    # 평가 수행
    eval_fn(eval_data_loader, model, enc_sentiment, enc_aspect, device, log, f1_mode='micro', flag=flag)
