...
def upload_directory_with_transfer_manager(bucket_name, source_directory, workers=1):
    ...
    results = transfer_manager.upload_many_from_filenames(
        bucket, string_paths, source_directory=source_directory, max_workers=workers, skip_if_exists=True # 덮어쓰기
    )
...
if __name__ == '__main__':    
    ...
    arg_parser.add_argument('--bucket_name', type=str, default='review_tagging', help='GCS bucket name (if not exists, create new bucket)')
    arg_parser.add_argument('--source_directory', type=str, default='./ckpt_review/model', help='source directory to upload')
    
    upload_directory_with_transfer_manager('review_tagging', './ckpt_review/model')